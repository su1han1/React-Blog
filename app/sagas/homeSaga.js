...
export function* login(username, password) {
    ...
    try {
        ...
        yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Username or Password doesn\'t match our records',msgType:0});
    } finally {
...
export function* register (data) {
    ...
    try {
        ...
        yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Sign up failed',msgType:0});
    } finally {
        yield put({type: IndexActionTypes.FETCH_END});
    }
}
...
export function* loginFlow() {
    while (true) {
        ...
        if(response&&response.code === 0){
            yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Successfully sign in',msgType:1});
...
export function* registerFlow () {
    while(true){
        ...
        if(response&&response.code === 0){
            yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Successfully sign up',msgType:1});
            yield put({type:IndexActionTypes.RESPONSE_USER_INFO,data:response.data})
        }
